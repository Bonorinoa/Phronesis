# Phronesis Project Manifest

project: Phronesis  
status: design-ready / implementation-pending  
generated: 2026-06-14  
root: `~/Hermes/Projects/Phronesis`  
github: `https://github.com/Bonorinoa/Phronesis`

## Canonical Documents

All canonical design documents live in `docs/`.

- `docs/00_README.md`
- `docs/01_Project_Overview.md`
- `docs/02_System_Architecture.md`
- `docs/03_Component_Design.md`
- `docs/04_Tool_Specifications.md`
- `docs/05_RAG_and_Ingestion.md`
- `docs/06_Context_Management.md`
- `docs/07_System_Prompt.md`
- `docs/08_Observability_and_Evaluation.md`
- `docs/09_Vibe_Engineering_Workflow.md`
- `docs/10_MVP_Scope_and_Development_Plan.md`
- `docs/11_Deployment_and_Hosting.md`
- `docs/12_Open_Questions_and_Future_Ideas.md`
- `docs/13_Glossary.md`
- `docs/14_Decision_Engineering_Log.md`

## Usage Note for Coding Agents

Before implementing any code for Phronesis, read at least:

1. `README.md`
2. `docs/PROJECT_MANIFEST.md`
3. `docs/01_Project_Overview.md`
4. `docs/02_System_Architecture.md`
5. `docs/04_Tool_Specifications.md`
6. `docs/05_RAG_and_Ingestion.md`
7. `docs/09_Vibe_Engineering_Workflow.md`
8. `docs/14_Decision_Engineering_Log.md`

`AGENTS.md` is intentionally absent until coding-agent conventions are discussed.
